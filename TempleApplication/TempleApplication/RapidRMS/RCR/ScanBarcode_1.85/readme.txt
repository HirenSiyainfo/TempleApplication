--------------------------------------------------------------------------------------------------------
Warning: iOS7.0 GM has currently 2 outstanding issues Apple are working on:

1. When iOS is left sleeping with Linea attached for extended period of time (overnight), no connection will be possible with the sled until the devices are detached and reattached or 
...
The workaround set to be active on iOS 7.0.x versions, hopefully it will be fixed at or before 7.1

2. Long periods of using Linea without restarting the program results in inability to properly work with the device until the program is closed and reopen. The time varies by how the devices are used,
...
The advice is to avoid upgrading to iOS7 at the moment until these issues are fixed.
