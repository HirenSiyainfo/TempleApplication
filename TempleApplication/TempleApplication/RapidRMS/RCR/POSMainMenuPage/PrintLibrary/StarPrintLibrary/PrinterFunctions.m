...
static SMPort *g_StarPort = nil;
static dispatch_queue_t g_PrintingQueue = NULL;
 ...
 */
+ (void)CheckStatusWithPortname:(NSString *)portName portSettings:(NSString *)portSettings sensorSetting:(SensorActive)sensorActiveSetting withDelegate:(id)delegate
{
    SMPort *starPort = nil;
    ActualPrinterStatus actualPrinterStatus;
    ActualDrawerStatus actualDrawerStatus;
    @try
    {
...
//        {
//            if (delegate) {
//                [delegate errorOccuredInGettingStatusWithTitle:@"Fail to Open Port" message:@"Please check printer setup."];
//            }
//            return;
//        }
//        usleep(1000 * 1000);
        ...
        if (status.offline == SM_TRUE)
        {
            ...
            {
                actualPrinterStatus = ActualPrinterStatusCoverIsOpen;
                message = [message stringByAppendingString:@"\nCover is Open"];
            }
            else if (status.receiptPaperEmpty == SM_TRUE)
            {
                actualPrinterStatus = ActualPrinterStatusOutOfPaper;
                message = [message stringByAppendingString:@"\nOut of Paper"];
            }
        }
        else
        {
            actualPrinterStatus = ActualPrinterStatusOnline;
        ...
        if (sensorActiveSetting == SensorActiveHigh)
        {
            ...
            if ([drawerStatus isEqualToString:@"Open"]) {
                actualDrawerStatus = ActualDrawerStatusOpen;
            }
            else {
                actualDrawerStatus = ActualDrawerStatusClose;
            }
        ...
        if (delegate) {
            [delegate actualDrawerStatus:actualDrawerStatus];
        }
        else {
            [PrintJob actualPrinterStatus:actualPrinterStatus];
            [PrintJob actualDrawerStatus:actualDrawerStatus];
        }
        return;
    }
    @catch (PortException *exception)
    {
        actualPrinterStatus = ActualPrinterStatusError;
        if (delegate) {
            [delegate errorOccuredInGettingStatusWithTitle:@"Printer Error" message:@"Error occured while getting staus.Please check printer setup." ];
        }
        else {
            [PrintJob actualPrinterStatus:actualPrinterStatus];
        }
...
+ (void)OpenCashDrawerWithPortname:(NSString *)portName portSettings:(NSString *)portSettings drawerNumber:(NSUInteger)drawerNumber
{
    ...
    [self sendCommand:commands portName:portName portSettings:portSettings timeoutMillis:10000 deviceName:@"Printer"];
}
...
+ (void)PrintCode39WithPortname:(NSString*)portName portSettings:(NSString*)portSettings barcodeData:(unsigned char *)barcodeData barcodeDataSize:(unsigned int)barcodeDataSize barcodeOptions:(BarCodeOptions)option height:(unsigned char)height narrowWide:(NarrowWide)width
{
    ...
    [self sendCommand:dataToSentToPrinter portName:portName portSettings:portSettings timeoutMillis:10000 deviceName:@"Printer"];
...
+ (void)PrintTextWithPortname:(NSString *)portName portSettings:(NSString*)portSettings slashedZero:(bool)slashedZero underline:(bool)underline invertColor:(bool)invertColor emphasized:(bool)emphasized upperline:(bool)upperline upsideDown:(bool)upsideDown heightExpansion:(int)heightExpansion widthExpansion:(int)widthExpansion leftMargin:(unsigned char)leftMargin alignment: (Alignment)alignment textData:(unsigned char *)textData textDataSize:(unsigned int)textDataSize
{
    ...
    [self sendCommand:commands portName:portName portSettings:portSettings timeoutMillis:10000 deviceName:@"Printer" withDelegate:nil];
...
+ (void)PrintImageWithPortname:(NSString *)portName portSettings:(NSString*)portSettings imageToPrint:(UIImage*)imageToPrint maxWidth:(int)maxWidth compressionEnable:(BOOL)compressionEnable withDrawerKick:(BOOL)drawerKick
{
    ...
    [self sendCommand:commandsToPrint portName:portName portSettings:portSettings timeoutMillis:10000 deviceName:@"Printer" withDelegate:nil];

    [commandsToPrint release];
}

+ (void)sendPrinterStatusForDevice:(NSString *)deviceName statusCode:(NSInteger)statusCode message:(NSString*)message usingDelegate:(id)delegate
{
    switch (statusCode) {
        case PSC_PRINT_JOB_COMPLETED:
            [delegate printerTaskDidSuccessWithDevice:deviceName];
            break;
        case PSC_PRINT_JOB_ERROR:
            [delegate printerTaskDidFailWithDevice:deviceName statusCode:statusCode message:message timeStamp:[NSDate date]];
            break;
        default:
            break;
    }
//    NSMutableDictionary *taskDictionary = [[NSMutableDictionary dictionary] retain];
//    [taskDictionary setValue:[NSDate date] forKey:@"TimeStamp"];
//    [taskDictionary setValue:deviceName forKey:@"Device"];
//    [taskDictionary setValue:message forKey:@"Message"];
//    [taskDictionary setValue:@(statusCode) forKey:@"StatusCode"];
//    
...
//        [defaultCenter postNotificationName:kPrintTaskNotification object:nil userInfo:taskDictionary];
//
//    });
//    
...
{
    static int count = 0;
    if (g_PrintingQueue == NULL) {
//        g_PrintingQueue = dispatch_queue_create("Print_Queue", DISPATCH_QUEUE_SERIAL);
        g_PrintingQueue = dispatch_get_main_queue();
    }
    
    if (g_PrintingQueue == NULL) {
        // Failed to start separate queue.
        // Let us try synchronized prining
        [self asynchSendCommand:commandsToPrint portName:portName portSettings:portSettings timeoutMillis:timeoutMillis];
    } else {
        [self sendPrinterStatusForDevice:deviceName statusCode:PSC_PRINT_JOB_DISPATCHED message:@"method dispatched" usingDelegate:delegate];
        dispatch_async(g_PrintingQueue, ^{
            [self sendPrinterStatusForDevice:deviceName statusCode:PSC_PRINT_JOB_STARTED message:@"Dispatched block executing" usingDelegate:delegate];
...
//            [self asynchSendCommand:commandsToPrint portName:portName portSettings:portSettings timeoutMillis:timeoutMillis];
         BOOL success =   [self __sendCommand:commandsToPrint portName:portName portSettings:portSettings timeoutMillis:timeoutMillis deviceName:deviceName withDelegate:delegate];
//            NSLog(@"Time to print or open drawer = %f", [[NSDate date] timeIntervalSinceDate:startDate]);
            [startDate release];
            NSLog(@"Write done");
            if (success)
            {
                [self sendPrinterStatusForDevice:deviceName statusCode:PSC_PRINT_JOB_COMPLETED message:@"Printing over" usingDelegate:delegate];
            }
            else
            {
            }
...
+ (void)asynchSendCommand:(NSData *)commandsToPrint portName:(NSString *)portName portSettings:(NSString *)portSettings timeoutMillis:(u_int32_t)timeoutMillis
{
//    @synchronized(self)
    

    {
//        [self __sendCommand:commandsToPrint portName:portName portSettings:portSettings timeoutMillis:timeoutMillis];
        if (!g_StarPort) {
            g_StarPort = [[self openPrinterPort:portName portSettings:portSettings timeoutMillis:timeoutMillis] retain];
        }
        if (g_StarPort) {
            
            BOOL printerOnline = NO;
            int count = 0;
            
            while ((count < 3) && (printerOnline == NO)) {
                count++;
                @try {
                    printerOnline = [g_StarPort getOnlineStatus];
                }
                @catch (NSException *exception) {
                    printerOnline = NO;
                    //                [SMPort releasePort:g_StarPort];
                    //                [g_StarPort release];
                    //                g_StarPort = nil;
                    sleep(1);
                }
            ...
            if (printerOnline) {
                [self __writeCommand:commandsToPrint port:g_StarPort];
                sleep(1);
            } else {
                [self displayAlertTitle:@"Fail to connect to printer" message:@"Please check if the printer is switched on. Check iPad settings if printer is connected to iPad."];
            }
        }
    }
}

+ (BOOL)__sendCommand:(NSData *)commandsToPrint portName:(NSString *)portName portSettings:(NSString *)portSettings timeoutMillis:(u_int32_t)timeoutMillis deviceName:(NSString *)deviceName withDelegate:(id)delegate
{
//    NSInteger commandSize = [commandsToPrint length];
//    unsigned char *dataToSentToPrinter = (unsigned char *)malloc(commandSize);
//    [commandsToPrint getBytes:dataToSentToPrinter]; // Change to be needed..
    
    int commandSize = (int)commandsToPrint.length;
    unsigned char *dataToSentToPrinter = (unsigned char *)malloc(commandSize);
    [commandsToPrint getBytes:dataToSentToPrinter length:commandSize];
    
    BOOL isPrinterTaskCompleted = TRUE;
    ...
    @try
    {
        ...
        if (starPort == nil)
        {
            isPrinterTaskCompleted = FALSE;

            [self sendPrinterStatusForDevice:deviceName statusCode:PSC_PRINT_JOB_ERROR message:@"Fail to Open Port" usingDelegate:delegate];

            [self displayAlertTitle:@"Fail to Open Port" message:@""];
            return isPrinterTaskCompleted;
        }
        
        NSLog(@"PF: Port created");
        
//  USE_BEGIN_CHECKED_BLOCK is must for printing the bill correctly.
#define USE_BEGIN_CHECKED_BLOCK
        
#ifdef USE_BEGIN_CHECKED_BLOCK
        StarPrinterStatus_2 status;

//        if ([deviceName isEqualToString:@"CashDrawer"])
//        {
//            
//        }
//        else
//        {
            ...
            if (status.offline == SM_TRUE) {
                ...
                [self sendPrinterStatusForDevice:deviceName statusCode:PSC_PRINT_JOB_ERROR message:@"Printer is offline" usingDelegate:delegate];
                
                [self displayAlertTitle:@"Error" message:@"Printer is offline"];
                return isPrinterTaskCompleted;
//            }
        }
        ...
        {
            int remaining = commandSize - totalAmountWritten; // Need to change here...
        ...
        {
            [self displayAlertTitle:@"Printer Error" message:@"Write port timed out"];
        }
        
        NSLog(@"PF: Data written to printer");
...
//            }
        ...
        NSLog(@"PF: EndCheckBlock");

    }
    @catch (PortException *exception)
    {
        NSLog(@"PortException = %@", exception);
        NSLog(@"PortException = %@", exception.description);
//        NSLog(@"PortException = %@", exception.);
        [self sendPrinterStatusForDevice:@"" statusCode:PSC_PRINT_JOB_ERROR message:@"Exception occured" usingDelegate:delegate];
        [self displayAlertTitle:@"Printer Error" message:@"Write port timed out"];
    }
    @finally
    {
...
+ (SMPort *)openPrinterPort:(NSString *)portName portSettings:(NSString *)portSettings timeoutMillis:(u_int32_t)timeoutMillis
{
    SMPort *starPort = nil;
    @try
    {
...
+ (void)__writeCommand:(NSData *)commandsToPrint port:(SMPort *)starPort
{
//    NSInteger commandSize = [commandsToPrint length];
//    unsigned char *dataToSentToPrinter = (unsigned char *)malloc(commandSize);
...
+ (void)PrintRasterSampleReceipt3InchWithPortname:(NSString *)portName portSettings:(NSString *)portSettings
{
    ...
    NSDictionary *textAttributes = @{NSFontAttributeName: [UIFont fontWithName:fontName size:fontSize]};
    ...
    CGRect messuredrect = [textToPrint boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin attributes:textAttributes context:nil];
    ...
    [textToPrint drawInRect:rect withAttributes:attributes];
...
//  fontSize *= multiple;
    ...
    style.alignment = NSLineBreakByWordWrapping;
    
    NSDictionary *dictionary = @{ NSFontAttributeName: [UIFont systemFontOfSize:18.0],
                                  NSParagraphStyleAttributeName: style};
    
    [textToPrint drawInRect:rect withAttributes:dictionary];
...
+ (SMBluetoothManager *)loadBluetoothSetting:(NSString *)portName portSettings:(NSString *)portSettings {
    ...
    if ([portName rangeOfString:@"BT:" options:NSCaseInsensitiveSearch range:NSMakeRange(0, 3)].location == NSNotFound) {
        [self displayAlertTitle:@"ERROR" message:@"This function is available via the bluetooth interface only."];
        return nil;
    }
    ...
    if (manager == nil) {
        [self displayAlertTitle:@"ERROR" message:@"initWithPortName:deviceType: is failure."];
        return nil;
    }
    
    if ([manager open] == NO) {
        [self displayAlertTitle:@"ERROR" message:@"open is failure."];
        return nil;
    }
    
    if ([manager loadSetting] == NO) {
        [self displayAlertTitle:@"ERROR" message:@"loadSetting is failure."];
        return nil;
    }
...
+ (void)disconnectPort:(NSString *)portName portSettings:(NSString *)portSettings timeout:(u_int32_t)timeout {
    ...
    if (result == NO) {
        [self displayAlertTitle:@"Fail to Disconnect" message:@""];
        return;
    }
...
+ (void)displayAlertTitle:(NSString*)title message:(NSString*)message {
    return;
