...
#import "PercentagePopUpVC.h"
#import "RmsDbController.h"

@interface PercentagePopUpVC () {
...
@implementation PercentagePopUpVC
...
- (IBAction) tenderNumPadButtonAction:(id)sender
{
    ...
    if (topinPrice.text != nil && topinPrice.text.length > 0) {
    ...
    if(topinPrice.text.length > 0)
    {
        if(!self.isQty)
        {
            ...
            topinPrice.text = [NSString stringWithFormat:@"%%%@",[currencyFormat stringFromNumber:dSales]];
            topinPrice.text = [topinPrice.text stringByReplacingOccurrencesOfString:@"," withString:@""];
        }
    }
}
