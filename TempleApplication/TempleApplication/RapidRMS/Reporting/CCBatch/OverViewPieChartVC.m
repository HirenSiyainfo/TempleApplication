...
#define HostView_Tag 252525

@interface OverViewPieChartVC ()
...
@implementation OverViewPieChartVC
...
-(void)configureHost {
    ...
    if (self.hostView) {
        [[self.view viewWithTag:HostView_Tag] removeFromSuperview];
    }
    // 2 - Create host view
    self.hostView = [(CPTGraphHostingView *) [CPTGraphHostingView alloc] initWithFrame:parentRect];
    self.hostView.allowPinchScaling = NO;
    self.hostView.tag = HostView_Tag;
...
-(NSString *)legendTitleForPieChart:(CPTPieChart *)pieChart recordIndex:(NSUInteger)index
{
    ...
    {
        NSString *LegendTitle = [NSString stringWithFormat:@"%@ (%@ / %@ / %@)",[(self.cardDetails)[index] valueForKey:@"Card" ],[self.rmsDbController applyCurrencyFomatter:[NSString stringWithFormat:@"%f",[[(self.cardDetails)[index] valueForKey:@"Amount"] floatValue]]],[(self.cardDetails)[index] valueForKey:@"TrnxCount"],[self.rmsDbController applyCurrencyFomatter:[NSString stringWithFormat:@"%f",[[(self.cardDetails)[index] valueForKey:@"AvgTicket"] floatValue]]]];
        return LegendTitle;
    }
    return @"N/A";
}
...
{
    NSString *LegendTitle = [NSString stringWithFormat:@"%@ (%@ / %@ / %@)",[(self.cardDetails)[idx] valueForKey:@"Card" ],[self.rmsDbController applyCurrencyFomatter:[NSString stringWithFormat:@"%f",[[(self.cardDetails)[idx] valueForKey:@"Amount"] floatValue]]],[(self.cardDetails)[idx] valueForKey:@"TrnxCount"],[self.rmsDbController applyCurrencyFomatter:[NSString stringWithFormat:@"%f",[[(self.cardDetails)[idx] valueForKey:@"AvgTicket"] floatValue]]]];
