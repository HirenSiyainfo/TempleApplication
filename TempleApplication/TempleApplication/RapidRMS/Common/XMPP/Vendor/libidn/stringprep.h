...
/* Error codes. */
  typedef NS_ENUM(unsigned int, Stringprep_rc)
  {
    STRINGPREP_OK = 0,
    /* Stringprep errors. */
...
/* Flags used when calling stringprep(). */
  typedef NS_OPTIONS(unsigned int, Stringprep_profile_flags)
  {
...
/* Steps in a stringprep profile. */
  typedef NS_ENUM(unsigned int, Stringprep_profile_steps)
  {
    STRINGPREP_NFKC = 1,
    STRINGPREP_BIDI = 2,
