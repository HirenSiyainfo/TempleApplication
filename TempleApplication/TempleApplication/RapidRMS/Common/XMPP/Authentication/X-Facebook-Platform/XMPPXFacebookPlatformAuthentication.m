...
@property (NS_NONATOMIC_IOSONLY, readonly, copy) NSString *base64EncodedFullResponse;
...
- (instancetype)initWithStream:(XMPPStream *)stream appId:(NSString *)inAppId accessToken:(NSString *)inAccessToken
{
    if ((self = [super init]))
    {
		xmppStream = stream;
...
- (XMPPHandleAuthResponse)handleAuth1:(NSXMLElement *)authResponse
{
	...
	response.stringValue = self.base64EncodedFullResponse;
