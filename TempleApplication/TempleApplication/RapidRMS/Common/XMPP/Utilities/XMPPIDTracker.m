...
- (instancetype)initWithDispatchQueue:(dispatch_queue_t)aQueue
{
	return [self initWithStream:nil dispatchQueue:aQueue];
}

- (instancetype)initWithStream:(XMPPStream *)stream dispatchQueue:(dispatch_queue_t)aQueue
{
...
- (void)addElement:(XMPPElement *)element trackingInfo:(id <XMPPTrackingInfo>)trackingInfo
{
	...
	trackingInfo.elementID = element.elementID;
    trackingInfo.element = element;
	[trackingInfo createTimerWithDispatchQueue:queue];
}
...
- (BOOL)invokeForElement:(XMPPElement *)element withObject:(id)obj
{
	...
	id <XMPPTrackingInfo> info = dict[element.elementID];
...
- (instancetype)initWithTarget:(id)aTarget selector:(SEL)aSelector timeout:(NSTimeInterval)aTimeout
{
    if(target || selector)
    {
...
- (instancetype)initWithBlock:(void (^)(id obj, id <XMPPTrackingInfo> info))aBlock timeout:(NSTimeInterval)aTimeout
{
