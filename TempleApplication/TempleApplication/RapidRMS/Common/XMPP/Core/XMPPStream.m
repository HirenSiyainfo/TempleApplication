...
**/
- (instancetype)initP2PFrom:(XMPPJID *)jid
{
    if ((self = [super init]))
    {
		// Common initialization
...
- (void)handleStreamFeatures
{
    ...
    else if(!self.secure && self.startTLSPolicy == XMPPStreamStartTLSPolicyRequired)
    {
	...
	if (f_bind)
	{
		...
		if (requestedResource.length > 0)
		{
			...
			resource.stringValue = requestedResource;
...
- (void)continueHandleBinding:(NSString *)alternativeResource
{
	if (alternativeResource.length > 0)
	{
		...
		NSXMLElement *resource = [NSXMLElement elementWithName:@"resource"];
		resource.stringValue = alternativeResource;
...
- (void)socket:(GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	if (tag == TAG_XMPP_WRITE_RECEIPT)
	{
		...
		XMPPElementReceipt *receipt = receipts.firstObject;
