...
{
	if(self.hasOutOfBandData && self.setIQ)
	{
		return YES;
	}else{
		return NO;
...
- (BOOL)isOutOfBandDataFailureResponse
{
	...
	if(self.hasOutOfBandData && self.errorIQ && errorCode == 404 && [errorType isEqualToString:@"cancel"])
	{
		return YES;
	}else{
		return NO;
...
- (BOOL)isOutOfBandDataRejectResponse
{
	...
	if(self.hasOutOfBandData && self.errorIQ && errorCode == 406 && [errorType isEqualToString:@"modify"])
	{
		return YES;
	}else{
		return NO;
