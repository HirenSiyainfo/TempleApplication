...
- (void)setBday:(NSDate *)bday {
	...
	elem.stringValue = bday.xmppDateString;
}
...
- (void)setTimeZone:(NSTimeZone *)tz {
	...
	if (tz != nil) {
		...
		elem.stringValue = [NSString stringWithFormat:@"%02ld:%02ld",
...
- (void)setLocation:(CLLocation *)geo {
	...
	if (geo != nil) {
		...
		lat.stringValue = [NSString stringWithFormat:@"%.6f", coord.latitude];
		lon.stringValue = [NSString stringWithFormat:@"%.6f", coord.longitude];
	} else if (elem != nil) {
		[self removeChildAtIndex:[self.children indexOfObject:elem]];
	}
}
...
- (void)setRevision:(NSDate *)rev {
	...
	elem.stringValue = rev.xmppDateTimeString;
}
