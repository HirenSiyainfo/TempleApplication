...
- (instancetype)initWithDispatchTimer:(dispatch_source_t)aTimer NS_DESIGNATED_INITIALIZER;
- (void)cancel;
...
- (instancetype)initWithCapabilitiesStorage:(id <XMPPCapabilitiesStorage>)storage dispatchQueue:(dispatch_queue_t)queue
{
	...
	for (NSXMLElement *field in fields)
	{
		...
		if ([var isEqualToString:@"FORM_TYPE"] && [type isEqualToString:@"hidden"])
		{
			...
			if (values.count > 0)
			{
				if (values.count > 1)
				{
					NSString *baseValue = [values.firstObject stringValue];
...
- (void)maybeQueryNextJidWithHashKey:(NSString *)key dueToHashMismatch:(BOOL)hashMismatch
{
	...
	NSUInteger requestIndex = [jids.firstObject unsignedIntegerValue];
...
- (instancetype)initWithDispatchTimer:(dispatch_source_t)aTimer
{
	if ((self = [super init]))
	{
		timer = aTimer;
