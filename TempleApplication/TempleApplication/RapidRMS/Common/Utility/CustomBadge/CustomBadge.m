...
// I recommend to use the allocator customBadgeWithString
- (instancetype) initWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining 
{
	self = [super initWithFrame:CGRectMake(0, 0, 25, 25)];
	if(self!=nil) {
		self.contentScaleFactor = [UIScreen mainScreen].scale;
...
+ (CustomBadge*) customBadgeWithString:(NSString *)badgeString
{
	...
	NSRange stringRange1 = {0, MIN([badgeString length], 2)};
	// adjust the range to include dependent chars
	stringRange1 = [badgeString rangeOfComposedCharacterSequencesForRange:stringRange1];
	// Now you can create the short string
	NSString *shortString1 = [badgeString substringWithRange:stringRange1];
	//Genrate the index of last word.
	NSUInteger index = badgeString.length-1;
	// Final string for display.
	NSString *finalString = [NSString stringWithFormat:@"%@...%@",shortString1,[badgeString substringFromIndex:index]];
	
	if (badgeString.length > 5) {
		return [[self alloc] initWithString:finalString withScale:1.0 withShining:YES];
	} else {
...
+ (CustomBadge*) customBadgeWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining
{
	...
	if (badgeString.length > 5) {
		return [[self alloc] initWithString:finalString withStringColor:stringColor withInsetColor:insetColor withBadgeFrame:badgeFrameYesNo withBadgeFrameColor:frameColor withScale:scale withShining:shining];
	} else {
...
- (void)drawRect:(CGRect)rect {
	...
	if (self.badgeText.length>0) {
		...
		if (self.badgeText.length<2) {
         ...
         NSDictionary *textAttributes = @{NSFontAttributeName: [UIFont boldSystemFontOfSize:sizeOfFont]};
        ...
        [self.badgeText drawAtPoint:CGPointMake((rect.size.width/2-textSize.width/2), (rect.size.height/2-textSize.height/2)) withAttributes:textAttributes];
	}
