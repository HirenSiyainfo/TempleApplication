...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL askDelegateForCredentials;
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL askDelegateForProxyCredentials;
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL useDataFromCache;
...
+ (void)initialize
{
	if (self == [ASIHTTPRequest class]) {
		...
		ASIRequestTimedOutError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIRequestTimedOutErrorType userInfo:@{NSLocalizedDescriptionKey: @"The request timed out"}] retain];	
		ASIAuthenticationError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIAuthenticationErrorType userInfo:@{NSLocalizedDescriptionKey: @"Authentication needed"}] retain];
		ASIRequestCancelledError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIRequestCancelledErrorType userInfo:@{NSLocalizedDescriptionKey: @"The request was cancelled"}] retain];
		ASIUnableToCreateRequestError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASIUnableToCreateRequestErrorType userInfo:@{NSLocalizedDescriptionKey: @"Unable to create request (bad url?)"}] retain];
		ASITooMuchRedirectionError = [[NSError errorWithDomain:NetworkRequestErrorDomain code:ASITooMuchRedirectionErrorType userInfo:@{NSLocalizedDescriptionKey: @"The request failed because it redirected too many times"}] retain];	
...
- (instancetype)initWithURL:(NSURL *)newURL
{
	...
	self.didReceiveResponseHeadersSelector = @selector(requestReceivedResponseHeaders:);
...
- (void)buildPostBody
{
	...
	if (self.postBodyFilePath) {
		...
		self.postLength = [[[NSFileManager defaultManager] attributesOfItemAtPath:path error:&err] fileSize];
		if (err) {
	...
	// Otherwise, we have an in-memory request body
		...
		if (self.shouldCompressRequestBody) {
			self.compressedPostBody = [ASIHTTPRequest compressData:self.postBody];
...
- (void)applyAuthorizationHeader
{
	...
	if (self.username && self.password && !self.domain) {
		...
		if (!credentials || (CFStringRef)credentials[@"AuthenticationScheme"] == kCFHTTPAuthenticationSchemeBasic) {
...
- (void)buildRequestHeaders
{
	...
	if (self.allowResumeForFileDownloads && self.downloadDestinationPath && self.temporaryFileDownloadPath && [[NSFileManager defaultManager] fileExistsAtPath:self.temporaryFileDownloadPath]) {
		NSError *err = nil;
		self.partialDownloadSize = [[[NSFileManager defaultManager] attributesOfItemAtPath:self.temporaryFileDownloadPath error:&err] fileSize];
		if (err) {
...
- (void)startRequest
{
	...
	if (self.shouldStreamPostDataFromDisk && self.postBodyFilePath && [[NSFileManager defaultManager] fileExistsAtPath:self.postBodyFilePath]) {
		...
		self.readStream = [(NSInputStream *)CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)self.postBodyReadStream) autorelease];
    } else {
		...
		if (self.postBody && self.postBody.length > 0) {
			...
			self.readStream = [(NSInputStream *)CFReadStreamCreateForHTTPRequest(kCFAllocatorDefault, request) autorelease];
		}
	}
	...
	if (self.shouldAttemptPersistentConnection) {
		...
		if (!self.connectionInfo && self.url.host && self.url.scheme) { // We must have a proper url with a host and scheme, or this will explode
			...
			for (NSMutableDictionary *existingConnection in persistentConnectionsPool) {
				if (!existingConnection[@"request"] && [existingConnection[@"host"] isEqualToString:self.url.host] && [existingConnection[@"scheme"] isEqualToString:self.url.scheme] && [existingConnection[@"port"]intValue] == self.url.port.intValue) {
...
- (void)checkRequestStatus
{
	...
	if (self.readStream) {
		...
		if (self.postLength) {
			...
			self.totalBytesSent = [NSMakeCollectable([(NSNumber *)CFReadStreamCopyProperty((CFReadStreamRef)self.readStream, kCFStreamPropertyHTTPRequestBytesWrittenCount) autorelease]) unsignedLongLongValue];
...
- (void)readResponseHeaders
{
	...
	self.responseHeaders = (NSDictionary *)headerFields;

	CFRelease(headerFields);
	
	self.responseStatusCode = (int)CFHTTPMessageGetResponseStatusCode(message);
	self.responseStatusMessage = [(NSString *)CFHTTPMessageCopyResponseStatusLine(message) autorelease];
	...
	if (self.shouldRedirect && [responseHeaders valueForKey:@"Location"]) {
		if ((self.responseStatusCode > 300 && self.responseStatusCode < 304) || self.responseStatusCode == 307) {
			...
			if (self.responseStatusCode != 307 && (!self.shouldUseRFC2616RedirectBehaviour || self.responseStatusCode == 303)) {
				...
				if (userAgent) {
					self.requestHeaders = [NSMutableDictionary dictionaryWithObject:userAgent forKey:@"User-Agent"];
				} else {
			...
			self.url = [NSURL URLWithString:[responseHeaders valueForKey:@"Location"] relativeToURL:self.url].absoluteURL;
...
- (void)parseStringEncodingFromHeaders
{
	...
	self.responseEncoding = encoding;
}
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		...
		self.proxyAuthenticationScheme = [(NSString *)CFHTTPAuthenticationCopyMethod(proxyAuthentication) autorelease];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(proxyAuthentication)) {
		self.proxyAuthenticationRealm = [(NSString *)CFHTTPAuthenticationCopyRealm(proxyAuthentication) autorelease];
	}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!requestAuthentication) {
		...
		CFRelease(responseHeader);
		self.authenticationScheme = [(NSString *)CFHTTPAuthenticationCopyMethod(requestAuthentication) autorelease];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		self.authenticationRealm = [(NSString *)CFHTTPAuthenticationCopyRealm(requestAuthentication) autorelease];
	}
...
- (void)handleBytesAvailable
{
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self.delegate respondsToSelector:self.didReceiveDataSelector]) {
			...
			invocation.selector = self.didReceiveDataSelector;
...
{	
	...
	if (self.temporaryFileDownloadPath) {
		...
		} else if (self.responseCompressed) {
...
- (BOOL)useDataFromCache
{
	...
	if (theRequest.downloadDestinationPath) {
		...
		theRequest.rawResponseData = [NSMutableData dataWithContentsOfFile:dataPath];
	}
