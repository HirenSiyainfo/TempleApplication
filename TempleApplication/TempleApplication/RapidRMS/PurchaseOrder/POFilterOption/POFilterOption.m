...
@interface POFilterOption ()<RapidItemFilterTypeVCDeledate,RapidFilterSelectedListCellDeledate>
{
...
@implementation POFilterOption

- (void)viewDidLoad {
    [super viewDidLoad];
    [[NSUserDefaults standardUserDefaults] setObject:@{} forKey:DEFAULT_FILTER_SAVED];
...
-(void)addFilterTypesView{
    if (!objFilterTypeVC) {
        ...
        objFilterTypeVC.arrFilterTypes = @[@(RapidItemFilterTypeDepartment),@(RapidItemFilterTypeVendor),@(RapidItemFilterTypeGroup),@(RapidItemFilterTypeTag)];
...
-(void)createKeyArrayForTableRows {
    ...
    NSArray * arrFilterType = @[@(PORapidItemFilterTypeDepartment),@(PORapidItemFilterTypeVendor),@(PORapidItemFilterTypeGroup),@(PORapidItemFilterTypeTag)];
